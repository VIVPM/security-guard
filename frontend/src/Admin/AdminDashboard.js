...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        axios
            .get('http://localhost:5000/admin/guards', {
    ...
    const handleConfirmDelete = async () => {
        try {
            await axios.delete(`http://localhost:5000/admin/guard/${guardToDelete._id}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
    ...
    const handleBgCheckSubmit = async () => {
        try {
            await axios.put(`http://localhost:5000/admin/guards/${selectedGuard._id}/background-check`, bgCheckData, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
            ...
            <Dialog open={addModalOpen} onClose={handleAddModalClose} fullWidth maxWidth="md">
                ...
                <DialogActions>
                    ...
                    <Button onClick={async () => {
                        ...
                        try {
                            await axios.post('http://localhost:5000/admin/guards', data, {
                            ...
                            axios.get('http://localhost:5000/admin/guards', {
                                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                            }).then((res) => setGuards(res.data));
                        } catch (err) {
